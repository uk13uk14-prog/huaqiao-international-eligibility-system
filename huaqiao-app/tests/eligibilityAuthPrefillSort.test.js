...
test('university list CSS: first card uses list gap, not margin collapsing; az inert when menu open', () => {
  ...
  assert.match(css, /\.univ-layout[\s\S]{0,280}padding-top:\s*12px/)
  ...
  assert.match(css, /body\.gq-univ-menu-open \.univ-screen \.mf-cell:not\(\.is-menu-open\)/)
...
test('university DOM title exists; recommend sort keeps first school first', () => {
  ...
  assert.match(app, /onUnivVantOpen/)
  assert.match(app, /gq-univ-menu-open/)
  ...
  assert.match(univSection, /teleport="body"/)
