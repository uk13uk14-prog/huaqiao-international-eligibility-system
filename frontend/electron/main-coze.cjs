...
const APP_NAME = '华侨生资格评估系统 Coze版';
...
/**
 * Get the path to the backend executable (Coze version)
 */
function getBackendPath() {
  if (app.isPackaged) {
    ...
    if (process.platform === 'win32') {
      return path.join(resourcesPath, 'huaqiao-backend-coze.exe');
    } else if (process.platform === 'darwin') {
      return path.join(resourcesPath, 'huaqiao-backend-coze');
    } else {
      return path.join(resourcesPath, 'huaqiao-backend-coze');
    }
  } else {
...
function getBackendEnv() {
  ...
  env.ENV = 'production';
  env.APP_VARIANT = 'coze'; // Mark as Coze variant
  
  // Set data directory (Coze-specific)
  if (process.platform === 'win32') {
    const appdata = process.env.APPDATA || path.join(process.env.USERPROFILE, 'AppData', 'Roaming');
    env.DATA_DIR = path.join(appdata, 'HuaqiaoEligibility-Coze');
  } else if (process.platform === 'darwin') {
    env.DATA_DIR = path.join(process.env.HOME, 'Library', 'Application Support', 'HuaqiaoEligibility-Coze');
  } else {
    env.DATA_DIR = path.join(process.env.HOME, '.local', 'share', 'huaqiao-eligibility-coze');
  }
...
function startBackend() {
  ...
  console.log(`[Coze] Starting backend: ${backendPath} ${backendArgs.join(' ')}`);
  ...
  try {
    ...
    backendProcess.stdout.on('data', (data) => {
      console.log(`[Coze Backend] ${data.toString().trim()}`);
    });
    
    backendProcess.stderr.on('data', (data) => {
      console.error(`[Coze Backend Error] ${data.toString().trim()}`);
    });
...
function stopBackend() {
  if (backendProcess) {
    console.log('[Coze] Stopping backend process...');
    ...
    setTimeout(() => {
      if (backendProcess) {
        console.log('[Coze] Force killing backend process...');
...
async function waitForBackend() {
  ...
  while (Date.now() - startTime < HEALTH_CHECK_TIMEOUT) {
    ...
    if (isReady) {
      console.log('[Coze] Backend is ready!');
      return true;
    }
...
function createLoadingWindow() {
  ...
  const loadingHtml = `
    ...
    <body>
      <div class="logo">🎓</div>
      <h1>华侨生资格评估系统</h1>
      <div class="version">Coze Edition v${APP_VERSION}</div>
