#!/usr/bin/env bash
# M1 PRODUCTION RELEASE — CSCA Exam Module V1 (Phase 3)
#
...
#
# Run ON M1 only (after explicit approval):
#   cd /Users/agent001/deploy/huaqiao-international-eligibility-system
#   git pull --ff-only origin cursor/mobile-cloud-preview
#   bash deploy/api/m1-csca-v1-production-release.sh
#
# Does: fingerprint → backup → alembic 008→009 → integrity → kickstart SaaS → CORS admin origin
...
#
# 009 SAFETY (hard):
#   Additive CSCA notification_rules only (24 rules). Never mutates universities (125),
#   admission timelines (900), users, membership, eligibility, or existing notifications.
#   Never rebuilds 008 tables.
#
...
run_checkpoint_d_diagnostic() {
  ...
  [[ -f "${BACKEND}/alembic/versions/009_csca_notification_rules.py" ]] || abort "missing 009 migration file"
...
echo "============================================================"
echo "GUOQIAO CSCA EXAM MODULE V1 — PHASE 3 PRODUCTION RELEASE"
echo "ROOT=${ROOT}"
...
git pull --ff-only origin cursor/mobile-cloud-preview
test -f "${BACKEND}/alembic/versions/009_csca_notification_rules.py" || abort "008 migration file missing after pull"
...
if [[ "${CUR_REV}" == "${EXPECTED_AFTER}" ]]; then
  info "Already at 009 — skip migrate (idempotent continue)"
  SKIP_MIGRATE=YES
else
  [[ "${CUR_REV}" == "${EXPECTED_BEFORE}" ]] || abort "can only migrate from ${EXPECTED_BEFORE} (008)"
  SKIP_MIGRATE=NO
fi
...
BACKUP_FILE="${BACKUP_DIR}/huaqiao_pre_009_csca_${TS}.dump"
CONTAINER_TMP="/tmp/huaqiao_pre_009_csca_${TS}.dump"
...
if [[ "${SKIP_MIGRATE}" != "YES" ]]; then
  section "CHECKPOINT D — APPLY 009"
  ...
  if [[ "${AFTER}" != "${EXPECTED_AFTER}" ]]; then
    ...
    abort "upgrade did not land on 009 (got ${AFTER})"
  fi
  ROLLBACK_REQUIRED=NO
  echo "ROLLBACK_REQUIRED=NO"
  echo "MIGRATION=PASS"
else
  echo "MIGRATION=SKIPPED_ALREADY_009"
fi
