#!/usr/bin/env bash
# Staging-only Alembic migrate for Admin AI Expert V1.
...
case "$STAGING_DATABASE_URL" in
  *5433*|*huaqiao_saas*|*@*/huaqiao[^\w]*|*@*/huaqiao)
    # allow huaqiao_admin_staging only; reject bare /huaqiao
...
case "$MODE" in
  up)
    ...
    ;;
  down)
    alembic_cmd downgrade 006_student_profile_slots
    alembic_cmd current
    ;;
  reup|cycle)
    echo "== upgrade head =="
    ...
    alembic_cmd current
    echo "== re-upgrade 007 =="
    alembic_cmd upgrade 007_admin_ai_expert_v1
    alembic_cmd current
    ;;
  *)
